...
export function createRelay(opts?: {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('chat:send', (req) => {
      ...
      // 비주얼 카드 트리거 — 메시지 텍스트, 또는 주사위 결과 라벨(성공/실패/단계)이 카드 이름과 정확히
      // 일치하면 전원 오버레이 재생. 후보 중 첫 매칭 하나만(한 발화에 컷인 1개). 발화 순간 1회만 emit 하므로
      // 히스토리 재생·재입장에는 다시 뜨지 않는다(클라 오버레이도 nonce 로 과거 트리거를 무시).
      const cardKeys =
        'text' in message && message.text
          ? [message.text]
          : message.kind === 'dice' && message.dice
            ? diceCardKeywords(message.dice)
            : []
